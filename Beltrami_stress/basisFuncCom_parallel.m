function [Idfuncs,lambdas,SLambda,dfuncs] = basisFuncCom_parallel(MM,m,covFunc,lx,ly,lz,sig_f,nhat,entry,exit,req,X,Y,Z,nsegs)
...
parfor pp = 1:n_p
    p_inds = (pp*p_size-p_size+1):pp*p_size;
for ss = 1:max(nsegs)
    segInd_tmp = find(nsegs(p_inds) >= ss);        % index of measurements that have at least this many segments
    segInd = p_inds(segInd_tmp);
