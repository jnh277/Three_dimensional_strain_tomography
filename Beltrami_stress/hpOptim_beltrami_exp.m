function [LogL,grad] = hpOptim_beltrami_exp(theta,num_basis,A,B,C,D,E,F,nu,entry,exit,y,sig_m,nsegs)
...
persistent Phi_save lambdas_save l_save

% if nargout == 2 && ~isempty(Phi_save)   % use old
if ~isempty(l_save) && all((l.^2 - l_save.^2)./l_save.^2 < 0.05) && size(Phi_save,1) == size(y,1)
% if false
    Phi = Phi_save;
...
if nargout == 2
    ...
    grad_tmp = [gradA;gradB;gradC;gradD;gradE;gradF];
